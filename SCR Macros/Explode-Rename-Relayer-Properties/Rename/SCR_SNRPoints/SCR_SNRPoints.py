...
class SCR_SNRPoints(StackPanel): # this inherits from the WPF StackPanel control
    ...
    def SetDefaultOptions(self):
        SCROptions.LoadProjectOptions(self, "SCR_SNRPoints", _OPTIONS, self.currentProject)

    def SaveOptions(self):
        SCROptions.SaveProjectOptions(self, "SCR_SNRPoints", _OPTIONS, self.currentProject)
                ...
                for o in self.objs:
                    ...
                    if isinstance(o, CoordPoint):
                        ...
                        if self.snrpoint.IsChecked:
                            ...
                            else: # manually setting FC from weird fc string - i.e. 'locator depth 1.6'

                                features = pm.AssociatedRDFeatures(o.SerialNumber)
                                depth = ""
                                if features.Count > 0:
                                    codestring = features[0].Code
                                    depth = str(float(re.sub("[^0-9\.]", "", codestring)))
                                    fcm.Delete(features[0])
                                    #tt2 = fcm.Remove(features[0]) # remove has a return value
                                    #features[0].Dispose() 
                                    #features[0].GetSite()
                                pm.SetFeatureCodeAtPoint(o.SerialNumber, "PPTU")
                                features = pm.AssociatedRDFeatures(o.SerialNumber)
                                for f in features: # sometimes it's unable to delete a weird feature code; needs to be done manually in properties pane
                                    if f.Code == "PPTU":
                                        for attr in f.Definition.AttributeDefinitions:
                                            if attr.Name == "Depth m":
                                                f.Add(attr.Type, attr.Name, depth)
                                        #for attr in f.Definition.AttributeDefinitions:
                                        #    if attr.Name == "Diameter m":
                                        #        f.Add(attr.Type, attr.Name, 0.765)
                                        for attr in f.Definition.AttributeDefinitions:
                                            if attr.Name == "Location Surveyed":
                                                f.Add(attr.Type, attr.Name, "Ground")
                                        for attr in f.Definition.AttributeDefinitions:
                                            if attr.Name == "Location Type":
                                                if depth != "":
                                                    f.Add(attr.Type, attr.Name, "CL")
                                                else:
                                                    f.Add(attr.Type, attr.Name, "CLH")
                                        #for attr in f.Definition.AttributeDefinitions:
                                        #    if attr.Name == "Material":
                                        #        f.Add(attr.Type, attr.Name, "Steel")
                                        for attr in f.Definition.AttributeDefinitions:
                                            if attr.Name == "Quality Level":
                                                if depth != "":
                                                    f.Add(attr.Type, attr.Name, "QL-B")
                                                else:
                                                    f.Add(attr.Type, attr.Name, "QL-C")
