...
class SCR_ViewFilterDialog(Window): # this inherits from the WPF StackPanel control
    def __init__(self, currentProject, macroFileFolder):
        ...
        self._suppress = 0  # reference-counted event suppression; handlers return early when > 0
        self._pending_mirror = set()
        self._mirror_dispatched = False
        ...
        if sender.EntitySerialNumbers.Count == sender.Content.Items.Count:
            ...
            if not sender.LastInputMethod == InputMethod.Programmatically:
                ...
                if sender.Name == "layerticklist1":
                    self._pending_mirror.add("2")
                elif sender.Name == "layerticklist2":
                    self._pending_mirror.add("1")
                elif sender.Name == "surfaceticklist1":
                    self._pending_mirror.add("4")
                elif sender.Name == "surfaceticklist2":
                    self._pending_mirror.add("3")

                if not self._mirror_dispatched:
                    self._mirror_dispatched = True
                    self.Dispatcher.BeginInvoke(
                        DispatcherPriority.Background,
                        Action(self._do_mirror_update)
                    )
                self._suppress -= 1
    ...
    def _do_mirror_update(self):
        self._mirror_dispatched = False
        boxtoupdate = ",".join(self._pending_mirror)
        self._pending_mirror = set()
        self.updateticksinlists(boxtoupdate)
    ...
    def setlayersfrommacro(self, sender):
        ...
        #if current_event_time - self.previous_event_time > 0.05 and not self.entityisdeleting and not self.undoredoinprogress:
        if not self.entityisdeleting and not self.undoredoinprogress:
    ...
    def SetDefaultOptions(self, sender, e):
        self.debug.Content = ''
        SCROptions.LoadWindowState(self, "SCR_ViewFilterDialog", default_width=1000, default_height=500)
        SCROptions.LoadProjectOptions(self, "SCR_ViewFilterDialog", _OPTIONS, self.currentProject)

    def SaveOptions(self, sender, e):
        SCROptions.SaveWindowState(self, "SCR_ViewFilterDialog")
        SCROptions.SaveProjectOptions(self, "SCR_ViewFilterDialog", _OPTIONS, self.currentProject)
