...
_OPTIONS = {
    "split": True, "combine": False, "move": False,
    "addentry": False, "createfrom3DFaces": False,
    "createfrompoints": False, "reversetriangulation": False,
    "deletefaces": False, "triggerinvisible": False,
    "combinedname": "", "bimtype": 0,
    "meshcolorpicker": TrimbleColor.ByLayer, "layerpicker": 8,
}
...
class SCR_SplitCombineIFCMeshes(StackPanel): # this inherits from the WPF StackPanel control
    ...
    _MODE_CONTROLS = {
        'split':                {'objs'},
        'combine':              {'objs', 'combinedname', 'Tree', 'bimtype', 'layerpicker'},
        'move':                 {'objs', 'Tree', 'triggerinvisible', 'bimtype'},
        'addentry':             {'objs', 'combinedname', 'Tree', 'bimtype', 'layerpicker'},
        'createfrom3DFaces':    {'objs', 'combinedname', 'Tree', 'meshcolorpicker', 'bimtype', 'layerpicker', 'deletefaces'},
        'createfrompoints':     {'objs', 'combinedname', 'Tree', 'meshcolorpicker', 'bimtype', 'layerpicker'},
        'reversetriangulation': {'meshpicker'},
    }
    _ALL_MODE_CONTROLS = {'objs', 'meshpicker', 'combinedname', 'Tree', 'triggerinvisible',
                          'meshcolorpicker', 'bimtype', 'layerpicker', 'deletefaces'}

    def radiochange(self, send, e):
        self.objs.IsEntityValidCallback = self.IsValidObject

        _modes = ['split', 'combine', 'move', 'addentry',
                  'createfrom3DFaces', 'createfrompoints', 'reversetriangulation']
        mode = next((m for m in _modes if getattr(self, m).IsChecked), None)
        if mode is None:
            return

        visible = self._MODE_CONTROLS[mode]
        for name in self._ALL_MODE_CONTROLS:
            show = name in visible
            ctrl = getattr(self, name)
            new_vis = Visibility.Visible if show else Visibility.Hidden
            if ctrl.IsEnabled != show:
                ctrl.IsEnabled = show
            if ctrl.Visibility != new_vis:
                ctrl.Visibility = new_vis
    ...
    def SetDefaultOptions(self):
        SCROptions.LoadMacroOptions(self, "SCR_SplitCombineIFCMeshes", _OPTIONS, self.currentProject)
        self.radiochange(None, None)

    def SaveOptions(self):
        SCROptions.SaveMacroOptions(self, "SCR_SplitCombineIFCMeshes", _OPTIONS)
