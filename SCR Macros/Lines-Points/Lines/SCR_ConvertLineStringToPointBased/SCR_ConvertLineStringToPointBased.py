...
_OPTIONS = {
    "newpoints": False, "useexisting": True, "matchFC": False,
    "relayerpoints": False, "drawcircles": True, "createnew": False,
}
...
class SCR_ConvertLinestringToPointBased(StackPanel): # this inherits from the WPF StackPanel control
    ...
    def OnLoad(self, cmd, buttons, event):
        ...
        SCRExpanders.wire_pairs([
            (self.expander_useexisting, self.useexisting),
        ])
    ...
    def SetDefaultOptions(self):
        SCROptions.LoadMacroOptions(self, "SCR_ConvertLineStringToPointBased", _OPTIONS)

    def SaveOptions(self):
        SCROptions.SaveMacroOptions(self, "SCR_ConvertLineStringToPointBased", _OPTIONS)
    ...
    def OkClicked(self, cmd, e):
        ...
        try:
            ...
            with TransactMethodCall(self.currentProject.TransactionCollector) as failGuard:
                ...
                GlobalSelection.Clear() # should make changing the selected lines much faster, otherwise the properties pane would update all the time

                time1 = datetime.now()
                for sn in selectionserials:
                    
                    #if (p1 * 100 // selectionserials.Count) % 10 == 0:
                    #tt = datetime.now() - time1 # delivers .days .microseconds . seconds
                    if (datetime.now() - time1).seconds > 1:
                        ProgressBar.TBC_ProgressBar.Title = "Convert Line " + str(p1) + '/' + str(selectionserials.Count)
                        if ProgressBar.TBC_ProgressBar.SetProgress(p1 * 100 // selectionserials.Count):
                            break
                        time1 = datetime.now()
                    ...
                    if isinstance(o, Linestring):
                        ...
                        for i in range(0, o.ElementCount):
                            ...
                            # 20 % increments                            
                            #if (i * 100 // o.ElementCount) % 50 == 0:
                            #    ProgressBar.TBC_ProgressBar.Title = "Line " + str(p1) + '/' + str(selectionserials.Count) + ' - Node ' + str(i + 1) + '/' + str(o.ElementCount)
                            #    if ProgressBar.TBC_ProgressBar.SetProgress((i + 1) * 100 // o.ElementCount):
                            #        break
                            ...
                            if "XYZ" in etype.Name:
                                ...
                                if not self.useexisting.IsChecked:
                                    ...
                                    if e.Position.Is3D:
                                        closebyserials = self.point_tree.find_within(e.Position.X, e.Position.Y, e.Position.Z, 0.001)
                                    else:
                                        closebyserials = []
