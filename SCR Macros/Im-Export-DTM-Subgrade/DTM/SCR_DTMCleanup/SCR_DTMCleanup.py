...
class SCR_DTMCleanup(StackPanel): # this inherits from the WPF StackPanel control
    ...
    def SetDefaultOptions(self):
        SCROptions.LoadMacroOptions(self, "SCR_DTMCleanup", _OPTIONS, self.currentProject)

    def SaveOptions(self):
        SCROptions.SaveMacroOptions(self, "SCR_DTMCleanup", _OPTIONS)
        ...
        try:
            ...
            with TransactMethodCall(self.currentProject.TransactionCollector) as failGuard:
                ...
                if surface.NumberOfTriangles > 2:
                    ...
                    for i in range(surface.NumberOfTriangles):
                        if surface.GetTriangleMaterial(i) != surface.NullMaterialIndex(): # only add triangles that are visible
                            for j in range(0, 3):
                                ...
                                vertextrirelations[tt].append(i)
                                trianglevertrelations[i].append(tt)
                                visiblevertices.Add(tt)
                    ...
                    ProgressBar.TBC_ProgressBar.Title = "Building Octree"
                    vpoints = {}
                    octree_items = []
                    for vi in visiblevertices:
                        p = surface.GetVertexPoint(vi)
                        vpoints[vi] = p
                        octree_items.append((p.X, p.Y, p.Z, vi))
                    vtree = SCROctree()
                    vtree.build(octree_items)
                    ...
                    cancelled = False
                    hztol = self.hztol.Distance
                    vztol = self.vztol.Distance
                    time1 = datetime.now()
                    for cnt in range(visiblevertices.Count - 1): # 1 less than the wholelist
                        ...
                        p0 = vpoints[v1]
                        sum_x = p0.X; sum_y = p0.Y; sum_z = p0.Z; count = 1
                        combinevertices = []
                        combinevertices.Add(v1)
                        averagep = p0

                        closebyvertices = vtree.find_within_box(
                            averagep.X - hztol, averagep.X + hztol,
                            averagep.Y - hztol, averagep.Y + hztol,
                            averagep.Z - vztol, averagep.Z + vztol)

                        for i in range(len(closebyvertices)):
                            if skipvertices[closebyvertices[i]] == True: continue
                            testp = vpoints[closebyvertices[i]]
                            if closebyvertices[i] != v1 and \
                               Vector3D(averagep, testp).Length2D <= hztol and \
                               abs(averagep.Z - testp.Z) <= vztol:

                                combinevertices.Add(closebyvertices[i])
                                sum_x += testp.X; sum_y += testp.Y; sum_z += testp.Z; count += 1
                                averagep = Point3D(sum_x / count, sum_y / count, sum_z / count)
                    ...
                    if averagepoints.Count == 0:
                    ...
                    elif not cancelled and self.createnewsurface.IsChecked:
                        ...
                        for t in range(nTri):
                            ...
                            if len(triverts) == len(set(triverts)): # if we don't have duplicates
                                ...
                                orig_verts = trianglevertrelations[t]
                                ...
                                for i in range(3):
                                    ...
                                    if "A." in str(gvi):
                                        ...
                                        vp = vpoints[triverts[i]]

                                    v = builder.AddVertex(vp) # that results the index in the new surface
                                    mapVertices[orig_verts[i]] = v[0] # map the old vertex numbers to the new one
                                ...
                                for side in range(3):
                                    ...
                                    iVertexA = orig_verts[side]
                                    nextSide = side + 1
                                    if nextSide == 3:
                                        nextSide = 0
                                    iVertexB = orig_verts[nextSide]
