...
class SCR_CreateGeoRefWorldFiles(StackPanel): # this inherits from the WPF StackPanel control
    ...
    def SetDefaultOptions(self):
        SCROptions.LoadMacroOptions(self, "SCR_CreateGeoRefWorldFiles", _OPTIONS)

    def SaveOptions(self):
        SCROptions.SaveMacroOptions(self, "SCR_CreateGeoRefWorldFiles", _OPTIONS)
    ...
    def OkClicked(self, cmd, e):
        ...
        for o in self.objs:
            if isinstance(o, GeoReferencedImage) or isinstance(o, GeoReferencedImage3D):  # Trimble.Vce.Data.RawData.GeoReferencedImage
                ...
                imagename = os.path.splitext(os.path.basename(fullimagefn))[0]
                ...
                if fullwrldfn == "":
                    ...
                    if self.copytofolder.IsChecked and not self.renamefile.IsChecked:
                        ...
                        if str.upper(imagefiletype) != 'JPG' and self.converttojpg.IsChecked:
                            self.copyasjpg(fullwrldfn, fullimagefn, imagename)                            
                            self.success.Text += 'and copied as JPEG\n' + o.Name + ' to ' + self.targetfolder.Text + '\n\n'
                    ...
                    if self.copytofolder.IsChecked and self.renamefile.IsChecked:
                        ...
                        if str.upper(imagefiletype) != 'JPG' and self.converttojpg.IsChecked:
                            self.copyasjpg(fullwrldfn, fullimagefn, o.Name)                            
                            self.success.Text += 'and copied as JPEG\n' + o.Name + ' to ' + self.targetfolder.Text + '\n\n'
    ...
    def copyasjpg(self, fullwrldfn, fullimagefn, newimagename):
        
        image = Image.FromFile(fullimagefn)
        tt = os.path.join(self.targetfolder.Text, (newimagename + ".jpg"))
        
        if File.Exists(tt):
            File.Delete(tt)
        else:
            image.Save(tt, ImageFormat.Jpeg)
        
        shutil.copy2(fullwrldfn, os.path.join(self.targetfolder.Text, (newimagename + ".jgw")))                            
