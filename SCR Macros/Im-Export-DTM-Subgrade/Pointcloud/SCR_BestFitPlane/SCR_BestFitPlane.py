...
_OPTIONS = {
    "layerpicker":         8,
    "drawxyz":             False,
    "createplanesurface":  False,
    "limitplanesurface":   True,
    "limitbypoints":       True,
...
class SCR_BestFitPlane(StackPanel): # this inherits from the WPF StackPanel control
    ...
    def OnLoad(self, cmd, buttons, event):
        ...
        #self.samplingdistance.NumberOfDecimals = 3
        ...
        SCRExpanders.wire_pairs([
            (self.expander_createplanesurface, self.createplanesurface),
        ])
        try:
    ...
    def SetDefaultOptions(self):
        SCROptions.LoadMacroOptions(self, "SCR_BestFitPlane", _OPTIONS, self.currentProject)

    def SaveOptions(self):
        SCROptions.SaveMacroOptions(self, "SCR_BestFitPlane", _OPTIONS)
    ...
    def OkClicked(self, cmd, e):
        ...
        try:
            with TransactMethodCall(self.currentProject.TransactionCollector) as failGuard:
                ...
                rottozero = Spinor3D.ComputeRotation(Vector3D(centerp, centerp + vx), v, Vector3D(1,0,0), Vector3D(0,0,1))
                ...
                if self.createplanesurface.IsChecked:
                    ...
                    if self.limitplanesurface.IsChecked:
                        ...
                        if self.limitbypoints.IsChecked:
                        ...
                        elif self.limitbydistance.IsChecked:
                            # need to draw the boundary since we need it as object in order to limit the planar surface
                            dist = self.limitdistance.Distance
                            polyseg = PolySeg.PolySeg()

                            polyseg.Add(Point3D(-dist, -dist, 0))
                            polyseg.Add(Point3D(dist, -dist, 0))
                            polyseg.Add(Point3D(dist, dist, 0))
                            polyseg.Add(Point3D(-dist, dist, 0))
                            polyseg.Close(True)
                            polyseg.Transform(matrixback)
                    ...
                    slope.Name = Model3D.GetUniqueName(self.surfacename.Text, None, wv) #make sure name is unique
                    if self.limitplanesurface.IsChecked:
                    ...
                    self.surfacename.Text = PointHelper.Helpers.Increment(self.surfacename.Text, None, True)
                ...
                try:
                    if self.createplanesurface.IsChecked and self.limitbypoints.IsChecked:
